...
export function calculatePartHealth(
  ...
  if (!machineInfo) return 0; // Handle cases where the machine name is not found in machineData

  const { value } = part;

  if (!machineInfo[part.name]) return -1;
...
export function calculateMachineHealth(
  ...
  const totalScore = partScores.reduce((sum, score) => {
    if (score === -1) return sum;

    partsCounter++;
    return sum + score;
  }, 0);

  if (partsCounter === 0) return 0;
